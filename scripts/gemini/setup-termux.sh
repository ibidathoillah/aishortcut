...
# --- Install Gemini CLI (Termux-optimized fork) ---
step "Installing Gemini CLI"
if command -v gemini >/dev/null 2>&1; then
  substep "Already installed: $(gemini --version 2>/dev/null | head -1)"
else
  substep "Installing @mmmbuto/gemini-cli-termux..."
  npm install -g @mmmbuto/gemini-cli-termux@latest 2>/dev/null || {
    substep "Retrying with --ignore-scripts..."
    npm install -g @mmmbuto/gemini-cli-termux@latest --ignore-scripts
  }
  if command -v gemini >/dev/null 2>&1; then
    chmod +x "$(command -v gemini)" 2>/dev/null || true
    success "  Installed: $(gemini --version 2>/dev/null | head -1)"
  else
    warn "  gemini CLI installation may not have completed."
  fi
fi
...
if command -v gemini-cli-telegram >/dev/null 2>&1; then
  ...
  chmod +x "$(command -v gemini-cli-telegram)" 2>/dev/null || true
...
substep "--- Phantom Killer Deactivation ---"
info "  Option 1 — From Termux (wireless ADB, no root needed):"
info "    pkg install android-tools"
info "    adb pair 192.168.1.x:PORT   # from wireless debugging"
info "    adb connect 192.168.1.x:PORT"
info "    adb shell device_config set_sync_disabled_for_tests persistent"
info "    adb shell device_config put activity_manager max_phantom_processes 2147483647"
info "    adb shell settings put global settings_enable_monitor_phantom_procs false"
info ""
info "  Option 2 — From computer (USB ADB):"
info "    adb shell device_config set_sync_disabled_for_tests persistent"
info "    adb shell device_config put activity_manager max_phantom_processes 2147483647"
info "    adb shell settings put global settings_enable_monitor_phantom_procs false"
info ""
info "  Option 3 — Root (directly in Termux):"
info "    su -c /system/bin/device_config set_sync_disabled_for_tests persistent"
info "    su -c /system/bin/device_config put activity_manager max_phantom_processes 2147483647"
info "    su -c setprop persist.sys.fflag.override.settings_enable_monitor_phantom_procs false"
info ""
info "  Verify (root):"
